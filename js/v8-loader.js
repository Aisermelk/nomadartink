 ...
 *   8. Preenche as avaliações do Google (nota + comentários), se ativado
 *      no painel — ver convenção data-v8-reviews abaixo
 *
 * ================================================================
 ...
 * ================================================================
 *
 *   <div data-v8-reviews></div>
 *
 * Só aparece se "Ativar avaliações" estiver marcado no painel e o Worker
 * tiver o secret GOOGLE_PLACES_API_KEY configurado. Renderiza a nota geral
 * e até 5 comentários, como elementos simples (sem CSS embutido — estilize
 * com as classes v8-reviews-*, v8-review-item no CSS do próprio site).
 */
...
(function () {
  ...
  function fillFields(config) {
    document.querySelectorAll("[data-v8]").forEach((el) => {
      ...
      // valor existe: garante que o elemento fica visível, mesmo que
      // tenha vindo com "hidden" estático no HTML (padrão recomendado
      // pra evitar mostrar um placeholder quebrado antes do JS rodar)
      el.hidden = false;
  ...
  function applyMedia(media) {
    ...
    if (galleryEl) {
      if (media.galleryEnabled && Array.isArray(media.galleryImages) && media.galleryImages.length) {
        ...
        if (galleryEl.style.display === "none") galleryEl.style.display = "";
  ...
  function applyLocation(location) {
    ...
    if (mapsEl) {
      if (location.enabled && location.embed) {
        mapsEl.hidden = false;
        if (mapsEl.style.display === "none") mapsEl.style.display = "";
  ...
  function applyReviews(reviews, projectId) {
    const el = document.querySelector("[data-v8-reviews]");
    if (!el) return;
    ...
    fetch(`${API_URL}/api/public/reviews/${encodeURIComponent(projectId)}`)
      .then((r) => r.json())
      .then((res) => {
        ...
        const reviewsHtml = (res.reviews || [])
          .map((rv) => `
            <div class="v8-review-item">
              <div class="v8-review-author">${rv.author}</div>
              <div class="v8-review-stars">${"★".repeat(rv.rating)}</div>
              <p class="v8-review-text">${rv.text || ""}</p>
              <span class="v8-review-time">${rv.relativeTime || ""}</span>
            </div>
          `)
          .join("");

        el.innerHTML = `
          <div class="v8-reviews-summary">
            <span class="v8-reviews-stars">${stars}</span>
            <strong class="v8-reviews-rating">${(res.rating || 0).toFixed(1)}</strong>
            <span class="v8-reviews-total">(${res.totalReviews || 0} avaliações no Google)</span>
          </div>
          <div class="v8-reviews-list">${reviewsHtml}</div>
        `;
      })
      .catch(() => {
        el.style.display = "none";
  ...
  function wireForm(projectId, formspreeUrl) {
    ...
    form.addEventListener("submit", async () => {
      ...
      const phone = data.whatsapp || data.telefone || data.phone || "";
      const message = data.message || data.mensagem || (phone ? `WhatsApp: ${phone}` : "");
  ...
  fetch(`${API_URL}/api/public/config/${encodeURIComponent(PROJECT_ID)}`)
    ...
    .then((res) => {
      if (res.error) {
        ...
        document.dispatchEvent(new CustomEvent("v8loader:done", { detail: null }));
        return;
      }
      ...
      applyReviews(config.reviews, PROJECT_ID);
      ...
      // (ex: esconder campos [data-dynamic] que continuaram vazios).
      document.dispatchEvent(new CustomEvent("v8loader:done", { detail: config }));
    })
    .catch(() => {
